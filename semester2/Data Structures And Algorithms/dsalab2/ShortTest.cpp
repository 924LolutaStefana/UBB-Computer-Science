...
void testAll() {
	...
	IteratedList TEST = IteratedList();
	TEST.addToBeginning(4);
	TEST.addToEnd(5);
	TEST.addToBeginning(3);
	TEST.addToEnd(6);
	TEST.addToBeginning(2);
	TEST.addToEnd(7);
	ListIterator start = TEST.first();
	ListIterator end = TEST.first();
	end.next();
	end.next();
	TEST.removeBetween(start, end);
	assert(TEST.size() == 3);
